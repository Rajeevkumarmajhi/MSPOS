...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        // Kitchen bar permissions set

        if($request->has('table-index')){
            $permission = Permission::firstOrCreate(['name' => 'table-index']);
            if(!$role->hasPermissionTo('table-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('table-index');

        if($request->has('table-add')){
            $permission = Permission::firstOrCreate(['name' => 'table-add']);
            if(!$role->hasPermissionTo('table-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('table-add');

        if($request->has('table-edit')){
            $permission = Permission::firstOrCreate(['name' => 'table-edit']);
            if(!$role->hasPermissionTo('table-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('table-edit');

        if($request->has('table-delete')){
            $permission = Permission::firstOrCreate(['name' => 'table-delete']);
            if(!$role->hasPermissionTo('table-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('table-delete');



        if($request->has('kitchen-index')){
            $permission = Permission::firstOrCreate(['name' => 'kitchen-index']);
            if(!$role->hasPermissionTo('kitchen-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('kitchen-index');

        if($request->has('kitchen-create')){
            $permission = Permission::firstOrCreate(['name' => 'kitchen-create']);
            if(!$role->hasPermissionTo('kitchen-create')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('kitchen-create');

        if($request->has('kitchen-edit')){
            $permission = Permission::firstOrCreate(['name' => 'kitchen-edit']);
            if(!$role->hasPermissionTo('kitchen-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('kitchen-edit');

        if($request->has('kitchen-delete')){
            $permission = Permission::firstOrCreate(['name' => 'kitchen-delete']);
            if(!$role->hasPermissionTo('kitchen-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('kitchen-delete');

        if($request->has('bar-index')){
            $permission = Permission::firstOrCreate(['name' => 'bar-index']);
            if(!$role->hasPermissionTo('bar-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('bar-index');
    
            if($request->has('bar-create')){
                $permission = Permission::firstOrCreate(['name' => 'bar-create']);
                if(!$role->hasPermissionTo('bar-create')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('bar-create');
    
            if($request->has('bar-edit')){
                $permission = Permission::firstOrCreate(['name' => 'bar-edit']);
                if(!$role->hasPermissionTo('bar-edit')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('bar-edit');
    
            if($request->has('bar-delete')){
                $permission = Permission::firstOrCreate(['name' => 'bar-delete']);
                if(!$role->hasPermissionTo('bar-delete')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('bar-delete');


        // End kitchen bar permission setting
    ...
    public function resetPermission()
    {
        try{
            ...
            dd('permission cache cleared successfully');
        }catch(\Exception $e){
            dd($e->getMessage());
        }
    }
