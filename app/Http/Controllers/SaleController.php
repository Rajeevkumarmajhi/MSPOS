...
class SaleController extends Controller
{
    ...
    public function kitchenSale()
    {
        $sales = Sale::with('saleProducts')->where('product_type','Kitchen')->where('sale_status','3')->get();
        return view('sale.kitchen',compact('sales'));
    }

    public function updateCookingStatus(Request $request){
        $sale = Sale::where('id',$request->id)->firstOrFail();
        $sale->cooking_status = $request->status;
        $sale->save();
        return response(['status'=>true,'data'=>$sale]);
    }

    public function barSale()
    {
        $sales = Sale::with('saleProducts')->where('product_type','Bar')->where('sale_status','3')->get();
        return view('sale.bar',compact('sales'));
    }
    ...
    public function store(Request $request)
    {
        ...
        //     if($table){
        //         $sale = Sale::where('table_id',$table->id)->where('sale_status')->first();
        //         if($sale)
        //             return redirect()->back('custom_error','This table is already booked');
        //     }
        // }
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $recent_sale = Sale::with('table')->where([
                    ['sale_status', 1],
                    ['user_id', Auth::id()]
                ])->orderBy('id', 'desc')->whereDate('created_at',Carbon::now())->take(10)->get();
                $recent_draft = Sale::with('table')->where([
                    ['sale_status', 3],
                    ['user_id', Auth::id()]
                ])->orderBy('id', 'desc')->whereDate('created_at',Carbon::now())->take(10)->get();
            }
            else {
                $recent_sale = Sale::where('sale_status', 1)->orderBy('id', 'desc')->whereDate('created_at',Carbon::now())->take(10)->get();
                $recent_draft = Sale::with('table')->where('sale_status', 3)->orderBy('id', 'desc')->whereDate('created_at',Carbon::now())->take(10)->get();
            }
            $lims_coupon_list = Coupon::where('is_active',true)->get();
            $flag = 0;
            $tables = Table::where('status','Active')->get();
            if(auth()->user()->role_id==4){
                $waiters = User::where('id',auth()->user()->id)->get();
            }else{
                $waiters = User::where('role_id',4)->where('is_active',1)->where('is_deleted',0)->get();
            }
            return view('sale.pos', compact('tables','waiters','all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft', 'lims_coupon_list', 'flag'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function createSale($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            return view('sale.create_sale',compact('tables','waiters','lims_biller_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'lims_coupon_list', 'lims_product_list', 'product_number', 'lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function genInvoice($id)
    {
        ...
        $table = Table::where('id',$lims_sale_data->table_id)->first();
        ...
        return view('sale.invoice', compact('table','lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
